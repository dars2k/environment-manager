...
export const CreateEnvironmentDialog: React.FC = () => {
  ...
  const { environmentCreateDialogOpen } = useAppSelector(state => state.ui);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      dispatch(showSuccess('Environment created successfully'));
      handleClose();
    },
    onError: (error: any) => {
      const errorMessage = error.response?.data?.message || 'Failed to create environment';
  ...
  const handleClose = () => {
    dispatch(setEnvironmentCreateDialogOpen(false));
    setFormData({
      name: '',
      description: '',
      environmentURL: '',
  ...
  const handleSubmit = () => {
    ...
    if (formData.credentials.type === 'password' && !password) {
      setError('Password is required for password authentication');
      return;
    }

    if (formData.credentials.type === 'key' && !privateKey) {
  ...
  return (
    <Dialog
      open={environmentCreateDialogOpen}
      onClose={handleClose}
      maxWidth="md"
      fullWidth
    >
